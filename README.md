...
CI runs `ruff` (lint), `mypy` (types, on the core), the full test suite, and the
round-trip audit on every push. These properties are checked against the catalog
in this repo — clone it and reproduce them yourself.
...
Current suite: **564 passing, 25 skipped** (the skips are live-API smoke tests
