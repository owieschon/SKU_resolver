...
(the adversarial live-results log). Nothing touches the fact path without
...
the cutover protocol B-probing, which needs the live Agent pointed at the endpoint.
...
**First-probe protocol (the cutover protocol):** the cutover is now a MEASUREMENT
...
(the fault-injection plan) against the two endpoints/flows so the harness
...
4. *Re-prove live (say is the fact path):* the FULL adversarial-live suite re-run
   ...
   — no on-hand count). the adversarial live-results log refreshed.
...
GATE-WIRED converse path — 12/12 still CONTAINED. the adversarial live-results log.
...
## Portfolio extraction (2026-06)

The entries above were written during the original, NDA-bound development. This
work sample was extracted from that codebase; the decisions made during extraction:

- **Synthetic, grammar-enumerated catalog.** `data/catalog.csv` is generated by
  enumerating the public part-number grammar (`scripts/generate_catalog.py`) —
  valid family/diameter/length/finish combinations plus opaque accessory codes.
  It exercises the real engine end-to-end while containing **no real company's
  part numbers, descriptions, prices, customers, or vendor data**. The round-trip
  figure (96.96%) is therefore high partly by construction; the grammar's harder
  validation was on the real ~10K-SKU catalog, not reproducible here. (Alternative
  considered: ship a small hand-written fixture — rejected as too small to
  exercise the long-tail grammar paths the engine is built for.)
- **Secrets/PII scrub of tree AND history.** The full git history was scanned for
  keys, tokens, customer/financial data; the published history is clean.
- **Parser monolith → package.** The ~6K-line `part_number_parser.py` became a
  package (`tables` / `_patterns` / `_dispatch`); the decoder↔dispatch import
  cycle is broken with a lazy relative import.
- **CI type/lint gates + a deliberate mypy boundary.** ruff + mypy run on every
  push. mypy gates the correctness-critical packages (resolution core,
  fulfillment, resolution service, model-provider, observability); the gateway
  (deliberate no-asserts posture), FastAPI runtime, and `erp_harness` (dynamic
  ERP dicts) are excluded and checked incrementally — stated, not hidden.
- **`translate()` refactored** from one ~219-line function into six named
  resolution-path helpers (behavior-preserving; suite-verified).
- **Observability layer** added: OTel spans (`gateway.turn → resolve.turn →
  llm.<task>`, Phoenix-ready via OTLP), env-gated Sentry, structured logging at
  the fault seam, all off-by-default/fail-open/PII-scrubbed (`docs/OBSERVABILITY.md`).
- **Doc accuracy.** Two design docs that described subsystems not present in this
  repo (an adversarial-generator under `src/adversarial/`, a pilot-ingestion
  data-handling layer) were removed; the real never-invent attack coverage lives
  in `tests/test_resolution_adversarial.py` + `tests/test_gateway_adversarial.py`.
  Some entries above reference original-development artifacts (the cutover
  protocol, the fault-injection plan, the adversarial live-results log) that are
  not included in this work sample.

**Status (this repo):** full suite **585 passed, 10 skipped** on a clean
